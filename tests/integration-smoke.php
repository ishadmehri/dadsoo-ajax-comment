<?php
/**
 * Integration smoke test for Dadsoo Ajax Comment.
 *
 * Run with:
 ...
 */
function dadsoo_ajax_run_handler($method, array $post)
{
 ...
 */
function dadsoo_ajax_count_queries(callable $callback)
{
    global $wpdb;
...
$results['legacy_function_aliases'] = function_exists('dadsoo_agax_comment')
    && function_exists('elinweb_agax_comment')
    && dadsoo_agax_comment() === dadsoo_ajax_comment()
    && elinweb_agax_comment() === dadsoo_ajax_comment();
$results['oldest_shortcodes_registered'] = shortcode_exists('elin-agax-comments')
    && shortcode_exists('elin-agax-comment-form');
$results['previous_shortcodes_registered'] = shortcode_exists('dadsoo-agax-comments')
    && shortcode_exists('dadsoo-agax-comment-form');
$results['current_shortcodes_registered'] = shortcode_exists('dadsoo-ajax-comments')
    && shortcode_exists('dadsoo-ajax-comment-form');

// --- Elementor widgets -------------------------------------------------------

$widgets = \Elementor\Plugin::$instance->widgets_manager->get_widget_types();
$results['form_widget_registered'] = isset($widgets['dadsoo-ajax-comment-form']);
$results['comments_widget_registered'] = isset($widgets['dadsoo-ajax-comments']);
// Pages built with an older version of the plugin still carry the old widgetType.
$results['previous_form_widget_registered'] = isset($widgets['dadsoo-agax-comment-form']);
$results['previous_comments_widget_registered'] = isset($widgets['dadsoo-agax-comments']);
$results['oldest_form_widget_registered'] = isset($widgets['elinweb-agax-comment-form']);
$results['oldest_comments_widget_registered'] = isset($widgets['elinweb-agax-comments']);
$results['legacy_widgets_hidden_from_panel'] = isset($widgets['dadsoo-agax-comments'])
    && false === $widgets['dadsoo-agax-comments']->show_in_panel()
    && isset($widgets['elinweb-agax-comments'])
    && false === $widgets['elinweb-agax-comments']->show_in_panel();

// --- Fixture -----------------------------------------------------------------

$post_id = wp_insert_post(array(
    'post_title' => 'Dadsoo Ajax AJAX Test',
    'post_content' => '[dadsoo-ajax-comment-form][dadsoo-ajax-comments items="2"]',
    'post_status' => 'publish',
    'comment_status' => 'open',
));
...
$form_html = do_shortcode('[dadsoo-ajax-comment-form]');
$list_html = do_shortcode('[dadsoo-ajax-comments items="2"]');

$results['form_shortcode_renders'] = false !== strpos($form_html, 'dadsoo_ajax_submit_comment');
$results['form_shortcode_has_honeypot'] = false !== strpos($form_html, 'dadsoo_ajax_confirm');
$results['list_shortcode_renders'] = false !== strpos($list_html, 'data-items="2"');
$results['oldest_shortcode_renders'] = false !== strpos(do_shortcode('[elin-agax-comments items="2"]'), 'data-items="2"');
$results['previous_shortcode_renders'] = false !== strpos(do_shortcode('[dadsoo-agax-comments items="2"]'), 'data-items="2"');

// Two form widgets on one page must not produce duplicate element ids.
$two_forms = do_shortcode('[dadsoo-ajax-comment-form]') . do_shortcode('[dadsoo-ajax-comment-form]');
...
$custom = dadsoo_ajax_comment()->render_comments_list(array(
    'items' => 2,
...
$blank = dadsoo_ajax_comment()->render_comments_list(array(
    'items' => 2,
    'load_more_text' => '   ',
    'empty_text' => '',
));
...
$results['blank_label_falls_back'] = false !== strpos($blank, __('Load more comments', 'dadsoo-ajax-comment'))
    && false !== strpos($blank, __('No comments yet.', 'dadsoo-ajax-comment'));
...
$results['custom_icon_renders'] = false !== strpos(
    dadsoo_ajax_comment()->render_comments_list(array(
        'items' => 2,
...
$results['avatar_size_attribute'] = false !== strpos(
    dadsoo_ajax_comment()->render_comments_list(array('items' => 2, 'avatar_size' => 96)),
    'data-avatar-size="96"'
);
...
$nonce = wp_create_nonce('dadsoo-ajax-comment-nonce');

$loaded = null;
$queries = dadsoo_ajax_count_queries(function () use (&$loaded, $post_id, $nonce) {
    $loaded = dadsoo_ajax_run_handler('ajax_load_comments', array(
        'post_id' => $post_id,
        'offset' => 0,
        'items' => 10,
        '_wpnonce' => $nonce,
...
$page_1 = dadsoo_ajax_run_handler('ajax_load_comments', array('post_id' => $post_id, 'offset' => 0, 'items' => 3, '_wpnonce' => $nonce));
$page_2 = dadsoo_ajax_run_handler('ajax_load_comments', array('post_id' => $post_id, 'offset' => 3, 'items' => 3, '_wpnonce' => $nonce));
...
$vote_1 = dadsoo_ajax_run_handler('ajax_comment_vote', array(
...
$_COOKIE = array();
$vote_2 = dadsoo_ajax_run_handler('ajax_comment_vote', array(
    'comment_id' => $first_comment,
    'vote_type' => 'like',
    '_wpnonce' => $nonce,
));
...
dadsoo_ajax_run_handler('ajax_comment_vote', array('comment_id' => $first_comment, 'vote_type' => 'like', '_wpnonce' => $nonce));
$switched = dadsoo_ajax_run_handler('ajax_comment_vote', array('comment_id' => $first_comment, 'vote_type' => 'dislike', '_wpnonce' => $nonce));
...
$invalid_vote = dadsoo_ajax_run_handler('ajax_comment_vote', array(
    'comment_id' => $first_comment,
    'vote_type' => 'shrug',
    '_wpnonce' => $nonce,
));
...
// A vote recorded only in a pre-4.0 cookie (either the "agax" typo era or the
// original elinweb era) is still honoured once. Uses root comments other than
// $first_comment (root_ids[0]), which already has real vote activity above.
$legacy_test_comments = array('dadsoo_agax_vote_' => $root_ids[1], 'elinweb_agax_vote_' => $root_ids[2]);
foreach ($legacy_test_comments as $legacy_cookie_prefix => $legacy_comment) {
    $_COOKIE = array($legacy_cookie_prefix . $legacy_comment => 'like');
    update_comment_meta($legacy_comment, 'dadsoo_ajax_likes', 1);
    $legacy_vote = dadsoo_ajax_run_handler('ajax_comment_vote', array(
        'comment_id' => $legacy_comment,
        'vote_type' => 'like',
        '_wpnonce' => $nonce,
    ));
    $results[$legacy_cookie_prefix . 'vote_honoured'] = isset($legacy_vote['data']['status'])
...
$guest = dadsoo_ajax_run_handler('ajax_submit_comment', array(
    'name' => 'Guest Tester',
    'email' => 'guest@example.test',
    'comment' => 'Guest comment awaiting moderation.',
    'post_id' => $post_id,
    'action' => 'dadsoo_ajax_submit_comment',
    'dadsoo_ajax_nonce' => $nonce,
    '_wp_http_referer' => '/',
));
...
$trapped = dadsoo_ajax_run_handler('ajax_submit_comment', array(
    'name' => 'Spam Bot',
    'email' => 'bot@example.test',
    'comment' => 'Buy cheap things right now.',
    'post_id' => $post_id,
    'dadsoo_ajax_confirm' => 'http://spam.example',
    'dadsoo_ajax_nonce' => $nonce,
));
$results['honeypot_rejects_submission'] = isset($trapped['success']) && false === $trapped['success'];

// A bad nonce must be refused.
$bad_nonce = dadsoo_ajax_run_handler('ajax_submit_comment', array(
    'name' => 'Nonce Tester',
    'comment' => 'This should never be stored.',
    'post_id' => $post_id,
    'dadsoo_ajax_nonce' => 'not-a-real-nonce',
));
...
if (!empty($admins)) {
    ...
    $too_deep = dadsoo_ajax_run_handler('ajax_reply_comment', array(
        'parent_id' => $level_3,
        'post_id' => $post_id,
        'name' => 'Moderator Tester',
